...
```{r}
mir92a.table = read.table("/media/Helios_scStorage/Wesley/103773-001/cellranger/Seurat3_Analysis_David/mir92a-targets.csv", sep = ",", header = T)
...
```{r}
mir92Targets<-intersect(mir92a.table[1:30,]$Gene.Symbol,rownames(FirstInMan.combined@assays$RNA))
...
for (celltype in levels(FirstInMan.combined$celltypes)) {
  ...
  avg<-AverageExpression(FirstInMan.combined, assays = "RNA", features = mir92Targets)
  avg$RNA<-exp(avg$RNA)
  ...
  gg <- ggplot(df_melted, aes(x=variable, y=value)) + 
    geom_boxplot() + geom_jitter() + scale_y_continuous(limits = c(0,2)) + 
    labs(title = celltype)
  print(gg)

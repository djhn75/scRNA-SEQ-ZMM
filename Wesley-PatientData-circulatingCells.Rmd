...
```{r}
<<<<<<< HEAD
load(file = paste(outputFolder,"CD31.circulatingCells.notImputed.rData",sep = ""))
=======
...
ggplot(df.melt,aes(x=conditions,y=value,fill=conditions)) + 
  geom_violin(scale = "width",) + facet_wrap(~variable, scales = "free") + theme(strip.background = element_blank()) + scale_fill_manual(values = my_color_palette)
...
#generate TSNEPlots for the reviewer
TSNEPlot(object = CD31.circulatingCells, label=T, pt.size = 0.1, do.return=TRUE, cells.use = WhichCells(CD31.circulatingCells,ident = "Ctrl-Aged")) + scale_color_manual(values = my_color_palette[1])
TSNEPlot(object = CD31.circulatingCells, label=T, pt.size = 0.1, do.return=TRUE, cells.use = WhichCells(CD31.circulatingCells,ident = "Ctrl-Young"))+ scale_color_manual(values = my_color_palette[2])
TSNEPlot(object = CD31.circulatingCells, label=T, pt.size = 0.1, do.return=TRUE, cells.use = WhichCells(CD31.circulatingCells,ident = "HF"))+ scale_color_manual(values = my_color_palette[3])
```
...
df.tmp<-data.frame(conditions=CD31.circulatingCells@meta.data$condition)
for (i in genes) {
  df.tmp<-data.frame(df.tmp,CD31.circulatingCells@data[i,])
}
colnames(df.tmp)<-c("conditions",genes)
...
ggplot(subset(df.melt, subset = conditions!="Ctrl-Young"),aes(x=conditions,y=value,fill=conditions)) + 
  geom_violin(scale = "width",) + facet_wrap(~variable, scales = "free") + theme(strip.background = element_blank()) + scale_color_manual(values = my_color_palette[1])

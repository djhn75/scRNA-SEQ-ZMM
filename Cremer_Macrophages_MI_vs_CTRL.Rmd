...
outputFolder<-"/media/Helios_scStorage/Badder_Sebastian_HfPef_Model/104284//Seurat3/"
sink(file = "/media/Helios_scStorage/Badder_Sebastian_HfPef_Model/104284/Seurat3/Cremer_Macrophages_MI_David-Seurat3.rmd.log", append = TRUE, split = TRUE)
```

```{r}
# Quick load
rm(SeuratObjectList, SeuratObject.combined_ClusterTree, p1.p2, tmpList, SeuratObject.anchors)
save.image("/media/Helios_scStorage/Badder_Sebastian_HfPef_Model/104284//Seurat3/Cremer_Macrophages_MI_David-Seurat3.RData")
load("/media/Helios_scStorage/Badder_Sebastian_HfPef_Model/104284//Seurat3/Cremer_Macrophages_MI_David-Seurat3.RData")
```
...
Sample.Paths <- c("/media/Helios_scStorage/Badder_Sebastian_HfPef_Model/104284/starsolo/104284-001-001Solo.out/Gene/filtered/",
                  "/media/Helios_scStorage/Badder_Sebastian_HfPef_Model/104284/starsolo/104284-001-002Solo.out/Gene/filtered/",
                  "/media/Helios_scStorage/Badder_Sebastian_HfPef_Model/104284/starsolo/104284-001-003Solo.out/Gene/filtered/",
                  "/media/Helios_scStorage/Badder_Sebastian_HfPef_Model/104284/starsolo/104284-001-004Solo.out/Gene/filtered/",
                  "/media/Helios_scStorage/Badder_Sebastian_HfPef_Model/104284/starsolo/104284-001-005Solo.out/Gene/filtered/",
                  "/media/Helios_scStorage/Badder_Sebastian_HfPef_Model/104284/starsolo/104284-001-006Solo.out/Gene/filtered/")

Samplenames <- c("MI-D3-1","MI-D3-2","NO-MI-1","MI-D3-3","NO-MI-2","NO-MI-3")
```
...
SeuratObject.combined <- FindClusters(SeuratObject.combined, resolution = 0.35)
```
...
```{r}
SeuratObject.combined$seurat_clusters<-SeuratObject.combined$integrated_snn_res.0.35
Idents(SeuratObject.combined)<-"integrated_snn_res.0.35"
```
...
```{r}
VlnPlot(SeuratObject.combined, features = "H2-Ab1")
```
...
write.csv2(x = Marker.states, file = "/media/Helios_scStorage/Badder_Sebastian_HfPef_Model/104284/Seurat3/PseudotimeStateGenes.csv")
...
load(paste0(outputFolder,"Cremer_HFpEF-HFrEF_David-Seurat3.RData"))
```


#Volcano plot of DEG's in clusters 
```{r}
library(ggrepel)

SeuratObject.68913<-subset(SeuratObject.combined, idents=c("6","8","9","13"))
Idents(SeuratObject.68913)<-"condition"
DEG.68913.MI_noMi<- FindMarkers(SeuratObject.68913, ident.1 = "NO-MI", ident.2 = "MI-D3",assay = "RNA", slot = "data", logfc.threshold = 0.1, min.pct = 0.1)

VlnPlot(SeuratObject.68913, features = "Malat1")
pvalues<-do_cluster_t_test(SeuratObject.68913, DEG.68913.MI_noMi, group="condition",cluster = "orig.ident")
pvalues<-data.frame(clustered.ttest=unlist(pvalues))

mycolors <- c("blue", "red", "black")
names(mycolors) <- c("DOWN", "UP", "NO")
de<-merge(x = DEG.68913.MI_noMi, pvalues, by = "row.names")
...
de$diffexpressed <- "NO"
de$diffexpressed[de$avg_logFC > 0.6 & de$p_val_adj < 0.05] <- "UP"
de$diffexpressed[de$avg_logFC < -0.6 & de$p_val_adj < 0.05] <- "DOWN"

de$delabel <- NA
#de$delabel[de$diffexpressed != "NO"] <- de$Row.names[de$diffexpressed != "NO"]
de$delabel[de$Row.names == "Malat1"] <- de$Row.names[de$Row.names == "Malat1"]


p<-ggplot(data=de, aes(x=avg_logFC, y=p_val_adj, col=diffexpressed, label=delabel)) + geom_point() + theme_minimal() +  geom_text_repel()
p <- p + geom_vline(xintercept=c(-0.6, 0.6), col="red") +
...
de$diffexpressed <- "NO"
de$diffexpressed[de$avg_logFC > 0.4 & de$clustered.ttest < 0.05] <- "UP"
de$diffexpressed[de$avg_logFC < -0.4 & de$clustered.ttest < 0.05] <- "DOWN"
...
p<-ggplot(data=de, aes(x=avg_logFC, y=-clustered.ttest, col=diffexpressed, label=delabel)) + geom_point() + theme_minimal() +  geom_text_repel(size=6)
p <- p + geom_vline(xintercept=c(-0.4, 0.4), col="red", ) +
    geom_hline(yintercept=-0.05, col="red")
p <- p + scale_colour_manual(values = mycolors)
